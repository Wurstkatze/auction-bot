...
def add_points(user_id, amount):
    conn = get_connection()
    c = conn.cursor()
    c.execute("INSERT INTO users (user_id, points) VALUES (?, ?) ON CONFLICT(user_id) DO UPDATE SET points = points + ?",
...
def remove_points(user_id, amount):
    conn = get_connection()
    c = conn.cursor()
    c.execute("UPDATE users SET points = points - ? WHERE user_id = ? AND points >= ?", (amount, user_id, amount))
    affected = c.rowcount
    conn.commit()
    conn.close()
    return affected > 0
